...
def segment_subject_sam2(
    ...
    and confidence gate validation.
    Generates and records all candidate masks, exports candidate_masks_contact_sheet.png and
    candidate_selection.json when hash_dir is provided.
    Raises ValueError if subject selection is ambiguous or invalid.
    """
    h, w = depth_map.shape

    # 1. Generate deduplicated candidate masks
    raw_candidates = generate_sam2_candidate_masks(rgb_array, depth_map, predictor)

    if not raw_candidates:
        raise ValueError("SUBJECT SELECTION: AMBIGUOUS - No valid candidate masks generated by SAM 2.")

    # 2. Extract features
    cand_features = []
    for idx, cand in enumerate(raw_candidates, start=1):
        feat = compute_candidate_features(cand, depth_map, candidate_id=idx)
        cand_features.append(feat)

    # 3. Score and rank candidates
    ranked_candidates = score_and_rank_candidates(cand_features, depth_map, h, w)

    # 4. Check for multi-object grouping
    grouped_ranked = check_and_group_candidates(ranked_candidates, depth_map, h, w)

    # 5. Evaluate confidence gate
    is_valid, selected_cand, confidence, margin, status_reason = evaluate_subject_selection_confidence_gate(grouped_ranked)

    selected_id = selected_cand["id"] if selected_cand else None

    # 6. Save diagnostic artifacts if hash_dir is provided
    if hash_dir is not None:
        hash_dir.mkdir(parents=True, exist_ok=True)
        contact_sheet = generate_candidate_masks_contact_sheet(rgb_array, grouped_ranked, selected_id)
        Image.fromarray(contact_sheet).save(hash_dir / "candidate_masks_contact_sheet.png")

        export_candidate_selection_json(
            hash_dir, grouped_ranked, selected_cand, is_valid, confidence, margin, status_reason
        )

    if not is_valid or selected_cand is None:
        raise ValueError(f"SUBJECT SELECTION: AMBIGUOUS - {status_reason}")

    subject_mask = selected_cand["mask_bool"]
    validate_subject_mask(subject_mask, (h, w))
    return subject_mask
